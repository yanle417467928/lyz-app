...
public class MaStoreRestController extends BaseRestController {
     ...
     * @date 2018/2/28
     */
    @GetMapping(value = "/findSmallFitAndStoresListByCityIdAndStoreId/{cityId}")
    public List<SimpleStoreParam> findSmallFitAndStoresListByCityIdAndStoreId(@PathVariable(value = "cityId") Long cityId) {
        logger.info("findSmallFitAndStoresListByCityIdAndStoreId 后台查询该城市ID的门店列表(下拉框) 入参 cityId:{}", cityId);
        try {
            //查询登录用户门店权限的门店ID
            List<Long> storeIds = this.adminUserStoreService.findStoreIdList();
            List<SimpleStoreParam> storesList = this.maStoreService.findSmallFitAndStoresListByCityIdAndStoreId(cityId, storeIds);
            logger.info("findSmallFitAndStoresListByCityIdAndStoreId ,后台查询该城市ID的门店列表(下拉框)成功", storesList);
            return storesList;
        } catch (Exception e) {
            e.printStackTrace();
            logger.warn("findSmallFitAndStoresListByCityIdAndStoreId EXCEPTION,发生未知错误，后台查询该城市ID的门店列表(下拉框)失败");
            logger.warn("{}", e);
            return null;
        }
    }
     ...
     * @date 2018/2/28
     */
    @GetMapping(value = "/findSmallFitStoresListByStoreIdNotBillRule")
    public List<SimpleStoreParam> findSmallFitStoresListByStoreIdNotBillRule() {
        logger.info("findSmallFitAndStoresListByCityIdAndStoreId 后台查询查询未生成规则的装饰公司(下拉框) ");
        try {
            //查询登录用户门店权限的门店ID
            List<Long> storeIds = this.adminUserStoreService.findStoreIdList();
            List<SimpleStoreParam> storesList = this.maStoreService.findSmallFitStoresListByStoreIdNotBillRule( storeIds);
            logger.info("findSmallFitAndStoresListByCityIdAndStoreId ,后台查询查询未生成规则的装饰公司(下拉框)成功", storesList);
            return storesList;
        } catch (Exception e) {
            e.printStackTrace();
            logger.warn("findSmallFitAndStoresListByCityIdAndStoreId EXCEPTION,发生未知错误，后台查询查询未生成规则的装饰公司(下拉框)失败");
            logger.warn("{}", e);
            return null;
        }
    }
...
//            List<Long> storeIds = this.adminUserStoreService.findStoreIdList();
     ...
     * @return
     * @throws
     * @title 根据分公司和门店类型查询门店(下拉框)-销量报表
     * @descripe
     * @author liuh
     * @date 2018/2/28
     */
    @GetMapping(value = "/findStoresListByCompanyCodeAndStoreTypeForSale")
    public List<SimpleStoreParam> findStoresListByCompanyCodeAndStoreTypeForSale(String companyCode,String storeType) {
        logger.info("findStoresListByCompanyCodeAndStoreType 后台查询门店列表(下拉框)-销量报表");
        try {
            //查询登录用户门店权限的门店ID
            List<Long> storeIds = this.adminUserStoreService.findStoreIdList();
            List<SimpleStoreParam> storesList = this.maStoreService.findStoresListByCompanyCodeAndStoreTypeForSale(companyCode,storeType,storeIds);
            logger.info("findStoresListByCompanyCodeAndStoreType ,后台查询门店列表(下拉框)成功", storesList.size());
            return storesList;
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @date 2018/2/28
     */
    @GetMapping(value = "/findFitAndStoresListByStoreId")
    public List<SimpleStoreParam> findFitAndStoresListByStoreId(Long cityId,String storeType) {
        logger.info("findFitAndStoresListByStoreId 后台查询门店和小型装饰公司列表(下拉框)");
        try {
            //查询登录用户门店权限的门店ID
            List<Long> storeIds = this.adminUserStoreService.findStoreIdList();
            List<SimpleStoreParam> storesList = this.maStoreService.findFitAndStoresListByStoreId(cityId,storeType,storeIds);
            logger.info("findStoresListByStoreId ,后台查询门店列表(下拉框)成功", storesList.size());
            return storesList;
        } catch (Exception e) {
            e.printStackTrace();
